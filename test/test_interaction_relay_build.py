...
class InteractionRelayBuildTests(unittest.TestCase):
    def test_kconfig_is_explicitly_default_off(self) -> None:
        ...
        for option in (
                "TK_VIBEPULSE_INTERACTION_RELAY",
                "TK_VIBEPULSE_AGENT_STATUS_RELAY"):
            match = re.search(
                rf"config {option}\n(?P<body>.*?)(?=\nconfig |\nendmenu|\Z)",
                source, re.S)
